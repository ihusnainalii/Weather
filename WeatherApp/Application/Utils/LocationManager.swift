...
import UIKit
import MapKit
///All the credit goes to [https://github.com/RajanMaheshwari/LocationManager]
    ...
    private func reverseGeoCoding(location:CLLocation?) {
        CLGeocoder().reverseGeocodeLocation(location!, completionHandler: {(placemarks, error)->Void in
            ...
            if placemarks!.count > 0 {
                ...
                printIfDebug("Problem with the data received from geocoder")
            }
        })
    }
...
extension LocationManager: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        if let location = locations.last {
            ...
            if (locationAge > 5.0) {
                printIfDebug("old location \(location)")
                return
            }
